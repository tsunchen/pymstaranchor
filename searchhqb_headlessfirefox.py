...
def getdynamic_searchhqb_list():
    ...
    driver = Firefox(executable_path=r'E:\dataUSB\geckodriver-v0.21.0-win64\geckodriver.exe', firefox_options=firefox_options)
    ...
    for name, inbuystat, inprogress, interest, rangeday in zip(names, inbuystatus, inprogresses, interests, rangedays):
        ...
        rangeday = rangeday.text.strip().replace(u"投资期限","").replace(u"天","")
        ...
        if inbuystat == u"立即赚钱":
            ...
            data = [name, inbuystat, inprogress, interest+u" %@hqblicai.com", rangeday+u"天", delta, '\t']
            print '|'.join(data)
