...
def get_searchbym_list(kyw=None, page=1):
    ...
    #for name, inprogress, interest, rangeday in zip(names, inprogresses, interests, rangedays):
        ...
        if inbuystat != u"":
            ...
            data = ['',name, inbuystat, inprogress, interest+u" %@baiyimao.com", rangeday+u" 天", delta, '\t']
            data_o = ['',name, inbuystat, inprogress, interest+u" %@baiyimao.com", rangeday, delta, '\t']
            #print (name)
            #print (name.encode('utf-8'))
    ...
    #data_arr_buffer = np.array(arr_buffer)
    #res = pd.DataFrame(data=data_arr_buffer[1:,1:],index=data_arr_buffer[1:,0],columns=data_arr_buffer[0,1:])
    #print( res[['Inprogess','Interest','Delta']][res['Delta']>'0.9'] ) 
    #print( res[['Name','Inprogess','Interest','Delta']][res['Delta']>'0.9'] ) 
